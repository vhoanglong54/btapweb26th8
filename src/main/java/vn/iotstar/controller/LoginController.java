...
public class LoginController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req,HttpServletResponse resp)throws ServletException,IOException {
        ...
        Cookie[] cookies=req.getCookies();if(cookies!=null)for(Cookie c:cookies)if(Constant.COOKIE_REMEMBER.equals(c.getName())){User account=service.get(c.getValue());if(account!=null){req.getSession(true).setAttribute(Constant.SESSION_ACCOUNT,account);resp.sendRedirect(req.getContextPath()+"/waiting");return;}}
    ...
    protected void doPost(HttpServletRequest req,HttpServletResponse resp)throws ServletException,IOException {
        ...
        User user=service.login(username,password);if(user==null){show(req,resp,"Tài khoản hoặc mật khẩu không đúng");return;}
